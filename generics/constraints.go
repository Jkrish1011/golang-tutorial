...
Sometimes we need logic in the generic function to know something about the type on which it is being implemented upon.

Thus, constrainsts are just interfaces that allow us to write generics that only operate within the constraints of a given
interface type.
...
type stringer interface {
	// Any type which implements String() method on it.
	String() string
}
...
func chargeForLineItem[T lineItem](newItem T, oldItems []T, balance float64) ([]T, float64, error) {
	...
	if cost > balance {
		return retLineItem, newBalance, errors.New("insufficient funds")
	}
	retLineItem = append(oldItems, newItem)
	return retLineItem, balance - cost, nil
}
